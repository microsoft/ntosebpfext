...
// This tool demonstrates how to use the process_monitor.sys BPF program to monitor process creation and deletion.
// The process_monitor.sys BPF program is a simple example of a BPF program that uses the eBPF API to monitor process
// creation and deletion that is implemented in the process_monitor.c file. The process_monitor_history_callback
// function is called when a process is created or deleted. The process_monitor_history_callback function looks up the
// process name and command line from the process_map and command_map BPF maps and prints the process information to the
// console.
