...
#define NTOS_EBPF_EXT_DEVICE_NAME L"\\Device\\NtosEbpfExt"

// Driver global variables
static WDFDEVICE _ntos_ebpf_ext_device = NULL;
static BOOLEAN _ntos_ebpf_ext_driver_unloading_flag = FALSE;
DEVICE_OBJECT* _ntos_ebpf_ext_driver_device_object;

const char ntos_ebpf_ext_version[] = EBPF_VERSION " " GIT_COMMIT_ID;
...
static void
_ntos_ebpf_ext_driver_uninitialize_objects()
{
    _ntos_ebpf_ext_driver_unloading_flag = TRUE;

    ntos_ebpf_ext_unregister_providers();

    ntos_ebpf_ext_trace_terminate();

    if (_ntos_ebpf_ext_device != NULL) {
        WdfObjectDelete(_ntos_ebpf_ext_device);
    }
}

static _Function_class_(EVT_WDF_DRIVER_UNLOAD) _IRQL_requires_same_
    _IRQL_requires_max_(PASSIVE_LEVEL) void _ntos_ebpf_ext_driver_unload(_In_ const WDFDRIVER driver_object)
{
...
//
static NTSTATUS
_ntos_ebpf_ext_driver_initialize_objects(_Inout_ DRIVER_OBJECT* driver_object, _In_ const UNICODE_STRING* registry_path)
{
    NTSTATUS status;
    ...
    driver_configuration.EvtDriverUnload = _ntos_ebpf_ext_driver_unload;

    status = WdfDriverCreate(driver_object, registry_path, WDF_NO_OBJECT_ATTRIBUTES, &driver_configuration, &driver);
    if (!NT_SUCCESS(status)) {
        NTOS_EBPF_EXT_LOG_NTSTATUS_API_FAILURE(NTOS_EBPF_EXT_TRACELOG_KEYWORD_BASE, "WdfDriverCreate", status);
        goto Exit;
    }
    ...
    if (!device_initialize) {
        ...
        NTOS_EBPF_EXT_LOG_NTSTATUS_API_FAILURE(
            NTOS_EBPF_EXT_TRACELOG_KEYWORD_BASE, "WdfControlDeviceInitAllocate", status);
        goto Exit;
    }
    ...
    RtlInitUnicodeString(&ebpf_device_name, NTOS_EBPF_EXT_DEVICE_NAME);
    status = WdfDeviceInitAssignName(device_initialize, &ebpf_device_name);
    if (!NT_SUCCESS(status)) {
        NTOS_EBPF_EXT_LOG_NTSTATUS_API_FAILURE(NTOS_EBPF_EXT_TRACELOG_KEYWORD_BASE, "WdfDeviceInitAssignName", status);
        goto Exit;
    }

    status = WdfDeviceCreate(&device_initialize, WDF_NO_OBJECT_ATTRIBUTES, &_ntos_ebpf_ext_device);
    if (!NT_SUCCESS(status)) {
        NTOS_EBPF_EXT_LOG_NTSTATUS_API_FAILURE(NTOS_EBPF_EXT_TRACELOG_KEYWORD_BASE, "WdfDeviceCreate", status);
    ...
    _ntos_ebpf_ext_driver_device_object = WdfDeviceWdmGetDeviceObject(_ntos_ebpf_ext_device);

    status = ntos_ebpf_ext_register_providers();
    if (!NT_SUCCESS(status)) {
        goto Exit;
    }

    WdfControlFinishInitializing(_ntos_ebpf_ext_device);
...
DriverEntry(_In_ DRIVER_OBJECT* driver_object, _In_ UNICODE_STRING* registry_path)
{
    ...
    status = ntos_ebpf_ext_trace_initiate();
    if (!NT_SUCCESS(status)) {
        // Fail silently as there is no other mechanism to indicate this failure. Note that in this case, the
        // NTOS_EBPF_EXT_LOG_EXIT() call at the end will not log anything either.
        goto Exit;
    }
    ...
    status = _ntos_ebpf_ext_driver_initialize_objects(driver_object, registry_path);
    if (!NT_SUCCESS(status)) {
    ...
    NTOS_EBPF_EXT_LOG_MESSAGE(
        NTOS_EBPF_EXT_TRACELOG_LEVEL_VERBOSE, NTOS_EBPF_EXT_TRACELOG_KEYWORD_BASE, ntos_ebpf_ext_version);
